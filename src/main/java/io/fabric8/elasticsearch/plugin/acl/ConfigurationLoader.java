...
public class ConfigurationLoader {
    ...
    public Map<String, Tuple<Settings, Long>> load(final String[] events, long timeout, TimeUnit timeUnit) throws InterruptedException, TimeoutException {
        ...
        final Map<String, Tuple<Settings,Long>> rs = new HashMap<>(events.length);
        ...
        loadAsync(events, new ConfigCallback() {
            ...
            public void success(String type, Settings settings, Long version) {
                ...
                rs.put(type, new Tuple<Settings, Long>(settings, version));
                latch.countDown();
        ...
        if(!latch.await(timeout, timeUnit)) {
            ...
            throw new TimeoutException("Timeout after " + timeout + "" + timeUnit 
                    + " while retrieving configuration for " + Arrays.toString(events) + "(index=" + searchguardIndex + ")");
        }
