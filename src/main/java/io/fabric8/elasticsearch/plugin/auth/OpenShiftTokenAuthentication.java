 ...
 * will populate roles based on satisfying a SubjectAccessReview for static role
 * definitions
 * 
 * The backend is configured as follows in the sg_config.yml:
 * 
 * searchguard: authc: my_domain: enabled: true order: 1 http_authenticator:
 * challange: false type:
 * io.fabric8.elasticsearch.plugin.auth.OpenShiftTokenAuthentication
 * authentication_backend: type:
 * io.fabric8.elasticsearch.plugin.auth.OpenShiftTokenAuthentication config:
 * subjectAccessReviews: prometheus: namespace: openshift-logging verb: view
 * resource: prometheus group: metrics.openshift.io
 */
public class OpenShiftTokenAuthentication implements AuthenticationBackend, HTTPAuthenticator {

    public static final String OPENSHIFT_AUTHENTICATOR = "openshift.authenticator.oauthtokenauthentication";
    ...
    public AuthCredentials extractCredentials(RestRequest request, ThreadContext context)
        ...
        if (PluginServiceFactory.getContextFactory() != null) {
            ...
            try {
                ...
                context.putTransient(OPENSHIFT_AUTHENTICATOR, true);
                return new AuthCredentials(requestContext.getUser(), requestContext).markComplete();
    ...
    public User authenticate(AuthCredentials credentials) throws ElasticsearchSecurityException {
        if (credentials.getNativeCredentials() != null
                && credentials.getNativeCredentials() instanceof OpenshiftRequestContext) {
            OpenshiftRequestContext context = (OpenshiftRequestContext) credentials.getNativeCredentials();
            User user = new User(context.getUser());
            addGeneralRoles(user, credentials, context);
            addSubjectAccessReviewRoles(user, context);
            return user;
        }
        return null;
    }

    private void addSubjectAccessReviewRoles(User user, OpenshiftRequestContext context) {
        final SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
            sm.checkPermission(new SpecialPermission());
        }
        if (PluginServiceFactory.getApiService() != null) {
                    ...
                    public Boolean run() {
                        try {
                            ...
                            return apiService.localSubjectAccessReview(context.getToken(), params.get("namespace"),
                                    params.get("verb"), params.get("resource"), params.get("group"),
