...
public class KibanaSeed implements ConfigurationSettings {
    ...
    private Tuple<Boolean, Project> seedUsersIndexPatterns(final OpenshiftRequestContext context, final String kibanaVersion) {
        ...
        Project defaultProject = projectsWithIndices.isEmpty() ? null : projectsWithIndices.get(0);
    ...
    private List<Project> filterProjectsWithIndices(Set<Project> projects) {
        ...
        SearchResponse response = pluginClient.search(patterns.toArray(new String[]{}), ArrayUtils.EMPTY_STRING_ARRAY);
        for (SearchHit hit : response.getHits().getHits()){
            result.add(kibanaUtils.getProjectFromIndexPattern(hit.getIndex()));
        }
